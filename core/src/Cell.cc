...
auto Cell::isEmpty() const -> bool { return !m_Content.has_value(); }
auto Cell::isDisabled() const -> bool { return m_Disabled; }
auto Cell::value() const -> Symbol {
  CORE_ASSERT(m_Content.has_value(), "value() called on empty cell");
  return m_Content.value();
}
void Cell::disable() { m_Disabled = true; }
void Cell::enable() { m_Disabled = false; }
